dispFields = dplyr::tibble(
  ...
  , enabled = c(T, T, F, F, T)
  , numericOk = c(T, T, F, F, F)
  , maxCardinality = c(Inf, Inf, 15, 8, 10)
)
...
dimAssignmentServer <- function(input, output, session, df, colDescs, dimParams) {
  dims <- dispFields
  choices <- names(df)
  ...
  getDimAssignments <- function() {
    ...
    dims$enabled <- vapply(
        dims$var, 
        function(dn) {
          id <- glue::glue('enabled_{dn}')
          ifelse(length(input[[id]]), input[[id]], dims[dims$var==dn,'enabled'][[1]])
        }, F)
    return(dims)
  }
  ...
  addSelectField <- function(dimRow, els) {
    ...
    if(!dimRow['required']) {
      # for display dimensions that aren't required, provide a checkbox
      #   to control whether they are enabled
      cbId <- glue::glue('enabled_{dimRow$var}')
      els <- c(els,
        list(shiny::checkboxInput(ns(cbId), label = dimRow$label, value = dimRow$enabled)))
